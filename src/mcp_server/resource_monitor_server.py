...
mcp = FastMCP("BrownieResourceMonitor")
...
class ResourceGuardianLogic:
    ...
    def check_for_stall(self, pid: int, last_progress_time: float, timeout_sec: int = 300) -> bool:
        """
        ...
        if cpu_usage > self.cpu_stall_threshold:
            ...
            if idle_time < timeout_sec + 300:
                logger.info(f"Task heartbeat stopped, but CPU is active ({cpu_usage}%). Assuming activity in progress.")
                return False
            else:
...
@mcp.tool()
async def check_stall(last_heartbeat: float, timeout_sec: int = 300, pid: Optional[int] = None) -> bool:
