        ...
        for attempt in range(max_retries):
            try:
                ...
                if isinstance(e, (requests.exceptions.ConnectionError, urllib3.exceptions.ProtocolError, http.client.RemoteDisconnected, ConnectionResetError)):
                    ...
                    is_retryable = (status == 429) or (status == 403 and "secondary" in str(e).lower())
    ...
    async def get_issues_to_process(self, repo_name: str) -> List[Any]:
        ...
        try:
            ...
            def _fetch():
                ...
                issues = repo.get_issues(state='open', assignee=username, sort='updated', direction='desc')
            ...
            def _create():
                ...
                try:
                    ...
                    if e.status == 422:
                        ...
                        if pulls.totalCount > 0: return pulls[0]
                    raise e
    ...
    async def get_mentions_to_process(self, repo_name: Optional[str] = None) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for n in notifications:
                if n.subject.type not in ["Issue", "PullRequest"]: continue
                
                issue_repo_name = n.repository.full_name
                if self.persistence: self.persistence.upsert_repository(issue_repo_name)
                if repo_name and issue_repo_name != repo_name: continue
                
                try:
                    url_parts = n.subject.url.split("/")
                    if not url_parts or not url_parts[-1].isdigit(): continue
                    issue_number = int(url_parts[-1])
                    
                    if any(r["repo_name"] == issue_repo_name and r["number"] == issue_number for r in results): continue
                    ...
                    if f"@{my_username.lower()}" in (issue.body or "").lower() and issue_author != my_username:
                        latest_mention = {
                            ...
                            "reactions": await asyncio.to_thread(self._get_reactions_summary, issue)
                        }
                    
                    try:
                        comments = await asyncio.to_thread(lambda: list(issue.get_comments()))
                        for comment in comments:
                            c_author = getattr(comment.user, "login", "").lower() if comment.user else ""
                            if f"@{my_username.lower()}" in (comment.body or "").lower() and c_author != my_username.lower():
                                if not latest_mention or comment.created_at > latest_mention["_created_dt"]:
                                    latest_mention = {
                                        ...
                                        "reactions": await asyncio.to_thread(self._get_reactions_summary, comment)
                                    }
                    except Exception as ce:
                        logger.warning(f"Comment scan failed: {ce}")

                    if issue.pull_request:
                        try:
                            pr = await asyncio.to_thread(issue.as_pull_request)
                            reviews = await asyncio.to_thread(lambda: list(pr.get_reviews()))
                            for review in reviews:
                                r_author = getattr(review.user, "login", "").lower() if review.user else ""
                                if review.body and f"@{my_username.lower()}" in review.body.lower() and r_author != my_username.lower():
                                    if not latest_mention or review.submitted_at > latest_mention["_created_dt"]:
                                        latest_mention = {
                                            ...
                                            "url": getattr(review, "url", None),
                                            "html_url": getattr(review, "html_url", None),
                                            "user_login": getattr(review.user, "login", None),
                                            "author_association": getattr(review, "author_association", None),
                                            "reactions": await asyncio.to_thread(self._get_reactions_summary, review)
                                        }
                            
                            r_comments = await asyncio.to_thread(lambda: list(pr.get_review_comments()))
                            for rc in r_comments:
                                rc_author = getattr(rc.user, "login", "").lower() if rc.user else ""
                                if f"@{my_username.lower()}" in (rc.body or "").lower() and rc_author != my_username.lower():
                                    if not latest_mention or rc.created_at > latest_mention["_created_dt"]:
                                        latest_mention = {
                                            "repo_name": issue_repo_name,
                                            "number": issue.number,
                                            "comment_id": f"rc-{rc.id}",
                                            "body": rc.body,
                                            "_created_dt": rc.created_at,
                                            "created_at": rc.created_at.isoformat() if rc.created_at else None,
                                            "updated_at": rc.updated_at.isoformat() if rc.updated_at else rc.created_at.isoformat(),
                                            "node_id": getattr(rc, "node_id", None),
                                            "url": rc.url,
                                            "html_url": rc.html_url,
                                            "user_login": getattr(rc.user, "login", None),
                                            "author_association": getattr(rc, "author_association", None),
                                            "reactions": await asyncio.to_thread(self._get_reactions_summary, rc)
                                        }
                        except Exception as pe:
                            logger.warning(f"PR detail scan failed: {pe}")
                    
                    if latest_mention:
                        results.append(latest_mention)
                except Exception as ie:
                    logger.error(f"Notification processing error: {ie}")
                    continue
            return results
        except Exception as e:
            logger.error(f"Mentions poll failed: {e}")
            return []
        ...
        try:
            ...
            for issue in issues_list:
                try:
                    ...
                    if f"@{my_username.lower()}" in (issue.body or "").lower() and issue_author != my_username:
                        latest_mention = {
                            "repo_name": repo_name, "number": issue.number, "comment_id": "body", "body": issue.body,
                            "_created_dt": issue.created_at, "created_at": issue.created_at.isoformat(),
                            "updated_at": issue.updated_at.isoformat(), "node_id": getattr(issue, "node_id", None),
                            "url": issue.url, "html_url": issue.html_url, "user_login": issue_author,
                            "reactions": await asyncio.to_thread(self._get_reactions_summary, issue)
                        }
                    
                    comments = await asyncio.to_thread(lambda: list(issue.get_comments()))
                    for c in comments:
                        c_author = getattr(c.user, "login", "").lower() if c.user else ""
                        if f"@{my_username.lower()}" in (c.body or "").lower() and c_author != my_username.lower():
                            if not latest_mention or c.created_at > latest_mention["_created_dt"]:
                                latest_mention = {
                                    "repo_name": repo_name, "number": issue.number, "comment_id": str(c.id), "body": c.body,
                                    "_created_dt": c.created_at, "created_at": c.created_at.isoformat(),
                                    "updated_at": c.updated_at.isoformat(), "node_id": getattr(c, "node_id", None),
                                    "url": c.url, "html_url": c.html_url, "user_login": getattr(c.user, "login", None),
                                    "reactions": await asyncio.to_thread(self._get_reactions_summary, c)
                                }
                    if latest_mention: found.append(latest_mention)
    ...
    async def ensure_repo_cloned(self, repo_name: str, repo_path: str, branch_name: Optional[str] = None):
        ...
        def _sync():
            ...
            try:
                ...
                subprocess.run(["git", "checkout", "-b", target, f"origin/{target}"], cwd=repo_path, check=True)
            subprocess.run(["git", "reset", "--hard", f"origin/{target}"], cwd=repo_path, check=True)
