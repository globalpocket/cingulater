...
Antigravity などのチャットインターフェースに対し、以下のように「ざっくりとしたタスクの丸投げ」が可能です。Brownie は指示の意図を汲み取り、自律的に解決策を模索します。

1. **面倒なリファクタリングの丸投げ**:
   > 「このあたりのコード、いい感じにリファクタリングして Brownie に投げといて。テストまで通れば OK だから。」
   > ※ 細かな指示をせずとも、Brownie が最適な設計を考え、サンドボックスで検証しながら完成させます。

2. **バックグラウンドでの並列処理**:
   > 「バックエンドの面倒なバグ修正、Brownie に任せた。結果だけあとで報告して。」
   > ※ 開発者は目の前のコーディングに集中し、時間のかかる調査や修正は裏側の Brownie に委ねます。

3. **プロジェクト全体のクリーンアップ**:
   > 「プロジェクト全体のコード整理を Brownie に頼めるかな？ 規約通りに直しておいてほしい。」
   > ※ 広範囲に及ぶルーチンワークを、信頼できるパートナーに一括してデリゲートします。
